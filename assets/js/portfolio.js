(function () {
  const translations = {
    ...
    zh: {
      ...
      "project.navaDescription": "NAVA 是一个面向生物节律与日常健康管理的 AI 应用，把移动端健康记录、HealthKit 数据、Supabase 服务和节律建模连接到一起。我主要负责 App 开发，同时作为算法团队成员协助节律模型迭代，参与多日健康信号的验证、解释与产品化呈现，把模型输出转化为用户能够理解和使用的健康洞察。",
